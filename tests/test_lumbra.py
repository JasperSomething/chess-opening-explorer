...
class ImportTests(unittest.TestCase):
    ...
    def test_missing_and_complete_local_overlay(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(data['moves'][0]['lumbra_percent'], 50)
            self.assertEqual(data['moves'][0]['lumbra2200_percent'], 100)
            all_view = lumbra.decorate(explorer.position(old, []), root/'local.sqlite', 'all')
            e4 = next(m for m in all_view['moves'] if m['san'] == 'e4')
            self.assertEqual(e4['reference_percent'], 50)
            self.assertEqual(e4['lumbra2200_percent'], 100)
            d4 = next(m for m in all_view['moves'] if m['san'] == 'd4')
            self.assertEqual((d4['lumbra'], d4['lumbra2200']), (1, 0))
            self.assertEqual((d4['lumbra_percent'], d4['lumbra2200_percent']), (50, 0))
